...
## 5A. 反証(verify)フェーズの結果と、**この監査自身の方法論的欠陥**

反証パスが完了した(71 エージェント)。**66件の判定のうち、生き延びたのは3件。**

| 判定 | 件数 |
|---|---:|
| `is_real = true`(生き延びた) | **3** |
| `is_real = false`(反証された) | 63 |

生き延びた3件は **Y54(fatal)/ Y53(medium)/ Y55(low)** として v1.7 で反映した。

### ⚠ 「63件が誤りだった」と読んではならない — 監査ハーネスの競合

**この監査には設計上の欠陥がある。**

```text
t0  監査を起動(対象ファイル = protocol v1)
t1  レンズが v1 を読んで指摘を出す
t2  ango が指摘を反映して v1.2 → v1.3 → v1.4 → v1.5 → v1.6 と改訂
t3  反証エージェントが「同じパス」を読む → 読んでいるのは v1.6
t4  反証エージェント:「この指摘は §6.3 で既に解決済み。よって is_real = false」
```

**反証エージェントは v1 に対する指摘を v1.6 に照らして判定している。**
実際、多くの反証理由は文字どおり
「これは Y1 / Y2 の再発見であり、v1.2 で修正済み」と述べている。
それは**指摘が誤りだった**という意味ではなく、**既に直っていた**という意味である。

したがって正しい読み方は:

> **63件の多くは「v1 に対しては正しく、v1.6 に対しては既に解決済み」である。**
> **反証パスが独立に検証したのは、主として「v1.7 時点でまだ残っている欠陥はどれか」**である。

**偽陽性率 95% と読むのは誤りであり、監査の価値を過小評価する。**
実際、v1.2〜v1.6 の改訂 57件は**この監査の指摘に基づいて行われた**ものである。

### この欠陥から得た運用ルール

1. **監査対象の artifact は、監査が完了するまで凍結する。**
   実行中に書き換えると、反証フェーズが別物を読む。
2. どうしても改訂するなら、**監査には commit hash を渡し、
   反証エージェントにもその hash を読ませる**。
3. 本監査の 63件は「反証された」ではなく **「v1.6 時点では該当しない」**と記録する。

**皮肉だが、これは Phase 7 confirmation §6.1 と同じ失敗である**:
あちらは fold 構成を途中で変えたために「窓が変わったのか手順が変わったのか」が
分からなくなった。こちらは監査中に対象を変えたために
「指摘が誤りなのか既に直したのか」が分からなくなった。
**比較の途中で比較対象を動かしてはいけない。**
