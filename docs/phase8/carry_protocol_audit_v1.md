...
| `replication_fidelity` | 論文の主張と設計が本当に対応しているか | **完了**(→ v1.6) |

**注記(正直な範囲宣言)**: 本記録は **5レンズすべて**を反映している。
ただし各指摘に対する**独立レビュアの反証判定は未完了**である(§5)。
v1.6 は「監査を通過した」のではなく「5レンズ分の指摘を反映した」状態である。

### レンズ別の検出数

| レンズ | fatal | high | medium | low | 反映先 |
|---|---:|---:|---:|---:|---|
| `leakage` | 2 | 6 | 4 | 1 | v1.2 |
| `statistics` | 4 | 6 | 4 | 1 | v1.2 |
| `contract_and_repo` | 2 | 6 | 6 | 2 | v1.3 |
| `economics` | 3 | 5 | 5 | 1 | v1.4 |
| `replication_fidelity` | 1 | 5 | 4 | 2 | v1.6 |

**重複を除いた反映件数: fatal 10・high 25・medium 19・low 6(計 60)。**
うち **3件は2つ以上のレンズが独立に同じ結論へ到達した**
(entry 規則の欠落 / B3 が単一実現値 / layer 1 gate が定性的)。

## 3A. `replication_fidelity` レンズ(v1.6 で反映)

**このレンズだけが「設計が論文の主張と本当に対応しているか」を見た。**

| id | 深刻度 | 指摘 |
|---|---|---|
| **Y46** | **fatal** | **A2 は2つの変種を検定している**(Table 7 Unrestricted / Table 8 Long-spot-only)。Arm R は Table 8 側だが、K8 に記録した Sharpe は別表の数値。**比較相手を取り違えていた** |
| **Y45** | high | **`basis_rel` = A2 の ρ という等号が誤り。** ρ は `κ(1−e^{−(f−s)}) − (r−r′)`、`κ=1095` で年率化され、**stablecoin 借入金利を含む** |
| **Y47** | high | **H11 を「未解決・fatal 相当」としていたが、A2 は Table 3 の caption に境界式を逐語で書いている**(`ρ_l = κ log(1−C)` / `ρ_u = κ log(1+C)`)→ **H11 解決** |
| Y48 | high | **A2 §4.2 は「funding はスプレッドを完全には追随しない」とも述べている**。§18.1 の R1 ゲートが厳しすぎると、**A2 自身が認める不完全さで replication 失敗**になる |
| Y49 | high | **layer 1 に下限が無い**(意図は 2020-01 だが `splits.assign` は 2023-11-19 より前を `None` にする) |
| **Y50** | medium | **momentum 交絡。** A2 は過去リターン momentum が futures–spot ギャップを **R² > 50%** で説明すると報告。**`ρ > ρ_u` で建てることは、過去リターンが高い局面で建てることに近い** — delta-neutral でも **entry タイミングは方向性を帯びる** |
| Y51 | medium | **comovement の主張が BTC 単独では検定不能**(黙って落とさず明記する) |
| Y52 | low | **K8 の表現が不正確**(「最大 3.5」は BTC の手数料ゼロ値であって資産横断の最大ではない) |

> **Y47 は「監査が未確定事項を1つ潰した」例である。**
> ango は H11 を fatal 相当の freeze blocker として人間へ投げようとしていたが、
> **答えは論文の表の caption に書いてあった。** 全文を読めば解決する問題を
> 人間の判断待ちにしていた。
