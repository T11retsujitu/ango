...
> 品質確認まで完了**した(ラベル未閲覧)。契約と実測値は
> [tier0_ingest_v1](tier0_ingest_v1.md)、機械可読レポートは
> `experiments/phase7/tier0_quality_v1.json`。
> §6 に従った **screening の事前登録も凍結済み**:
> [tier0_screening_preregistration_v1](tier0_screening_preregistration_v1.md)
> (family 27 test / 機械可読仕様は `src/mce/tier0_prereg.py`)。次は実行。
