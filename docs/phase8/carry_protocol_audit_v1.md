...
| レンズ | 観点 | 状態 |
|---|---|---|
| `leakage` | 先読み・情報漏洩・時刻規約 | **完了** |
| `statistics` | 帰無・多重比較・実効 N・検出力 | **完了** |
| `economics` | 執行現実性・資金・清算・両脚コスト | 実行中(本記録の対象外) |
| `contract_and_repo` | data contract・恒久ルール・既存コードとの整合 | 実行中(同上) |
| `replication_fidelity` | 論文の主張と設計が本当に対応しているか | 実行中(同上) |

**注記(正直な範囲宣言)**: 本記録は **完了した2レンズのみ**を反映している。
残り3レンズは未完了であり、**追加の欠陥が出る可能性がある**。
v1.2 は「監査を通過した」のではなく「2レンズ分の指摘を反映した」状態である。
